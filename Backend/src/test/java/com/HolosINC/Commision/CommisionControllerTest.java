package com.HolosINC.Commision;
...
public void testCreateCommisionSuccess() throws Exception {
    ...
    public void testChangeRequestedCommisionSuccess() throws Exception {
        // Dado
        CommisionDTO commisionDTO = new CommisionDTO(
                "Updated Test Commision",
                "Updated Description",
                100.0,
                EnumPaymentArrangement.FIFTYFIFTY,  // Usando el valor del Enum
                new byte[]{},
                new java.util.Date()
        );
        ...
        when(commisionService.requestChangesCommision(any(CommisionDTO.class), eq(COMMISION_ID)))
                .thenReturn(updatedCommision);

        // Ejecutar la prueba
        mockMvc.perform(put("/api/v1/commisions/" + COMMISION_ID + "/requestChanges")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(commisionDTO)))
        ...
        verify(commisionService, times(1)).requestChangesCommision(any(CommisionDTO.class), eq(COMMISION_ID));
    }
    @Test
    ...
    public void testGetCommisionByIdSuccess() throws Exception {
        ...
        // Simulando el comportamiento del servicio
        when(commisionService.getCommisionById(COMMISION_ID)).thenReturn(commision1);
    ...
    public void testCancelCommisionSuccess() throws Exception {
        // Simulando el comportamiento del servicio
        doNothing().when(commisionService).cancelCommision(COMMISION_ID, COMMISION_ID);
        ...
        verify(commisionService, times(1)).cancelCommision(COMMISION_ID, COMMISION_ID);
    }
}
